...
export const RuleMappingBaseSchema = ExperimentEvaluationRuleMapping.describe(
  "Maps an evaluator variable to a data source. `observation` rules use `input`, `output`, `metadata`; `experiment` rules also allow `expected_output` and `experiment_item_metadata`. Required for `llm_as_judge` evaluators; omit for `code` evaluators (EvalBear manages their mapping).",
);
