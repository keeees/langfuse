...
export default function AgentEvalCreatePage() {
  ...
  const fetchConfig = useCallback(async () => {
    ...
    try {
      ...
      setConfigReady(data.status?.agent_eval === "ready");
      if (data.agent_default_dataset_path)
        setDatasetPath(data.agent_default_dataset_path);
      if (data.default_limit) setLimit(data.default_limit);
      if (data.default_start_index != null)
        setStartIndex(data.default_start_index);
      if (data.judge_enabled_default != null)
  ...
  return (
    <Page headerProps={{ title: "Agent 评测" }} scrollable withPadding>
      <Head>
        <title>Agent 评测 | EvalBear</title>
      </Head>
      <div className="mx-auto max-w-lg space-y-4">
        <p className="text-muted-foreground text-sm">
          Agent 评测由 EvalBear 执行。每条 case 的输入、输出和 task_success
          分数会同步到 Langfuse 追踪，用于分析具体执行过程。
        </p>
        {configLoading ? (
          <LoadingSkeleton />
        ) : configReady === false ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="例如 /tmp/agent_trace_smoke_2.json"
                />
                <p className="text-muted-foreground text-xs">
                  JSON 数组格式，每条包含
                  case_id、input.task、expected.expected_outcome。
                </p>
              </div>
              <div className="space-y-2">
              ...
              <Button type="submit" className="w-full" disabled={isSubmitting}>
                {isSubmitting ? <LoadingSpinner /> : "开始 Agent 评测"}
              </Button>
            </form>
          </div>
        )}
